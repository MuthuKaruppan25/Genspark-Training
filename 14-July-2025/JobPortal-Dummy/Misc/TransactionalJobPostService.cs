...
public class TransactionalJobPostService : ITransactionalJobPostService
{
    ...
    public async Task<JobPostRegisterResponseDto> AddJobPostAsync(JobPostResponse jobPostDto)
    {
        ...
        try
        {
            ...
            var skillEntities = allSkills
                .Where(s => jobPostDto.skills.Any(ds => ds.Name.ToLower() == s.Name.ToLower()))
                .ToList();
            ...
            // 7. Add Locations
            if (jobPostDto.locations != null && jobPostDto.locations.Count > 0)
            {
                var locationEntities = jobPostDto.locations
