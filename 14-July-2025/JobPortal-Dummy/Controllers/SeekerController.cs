...
namespace JobPortal.Controllers
{
    ...
    public class SeekerController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateSeekerExperiences(string username, [FromBody] SeekerExperienceUpdateDto experienceDto)
        {
            if (experienceDto == null || experienceDto.experienceDetails == null || !experienceDto.experienceDetails.Any())
                return BadRequest("Experience details are required.");

            try
            {
                Console.WriteLine($"Received for: {username}");
                Console.WriteLine(JsonSerializer.Serialize(experienceDto));
                await _seekerService.UpdateSeekerExperiences(username, experienceDto, User);
                return Ok(new { message = "Seeker experiences updated successfully." });
            }
            catch (RecordNotFoundException ex)
            {
                return NotFound(new { error = ex.Message });
        ...
        [HttpPut("education/{username}")]
        public async Task<IActionResult> UpdateSeekerEducations(string username, [FromBody] SeekerEducationUpdateDto educationDto)
        {
            if (educationDto == null || educationDto.educationDetais == null || !educationDto.educationDetais.Any())
                return BadRequest("Education details are required.");

            try
            {
                await _seekerService.UpdateSeekerEducations(username, educationDto, User);
                return Ok(new { message = "Seeker education records updated successfully." });
            }
            catch (RecordNotFoundException ex)
            {
                return NotFound(new { error = ex.Message });
