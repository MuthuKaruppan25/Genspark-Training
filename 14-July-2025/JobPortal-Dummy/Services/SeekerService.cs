...
public class SeekerService : ISeekerService
{
    ...
    private readonly IRepository<Guid, Experience> _experienceRepository;
    private readonly IRepository<Guid, Education> _educationRepository;
    ...
    public SeekerService(
        ...
        IRepository<Guid, Experience> experienceRepository,
        IRepository<Guid,Education> educationRepository,
        ...
        try
        {
            ...
            var applicationDtos = applications
                ...
                .Select(app => new JobApplicationDto
                {
                    ...
                    Minsalary = app.jobPost?.Minsalary ?? 0,
                    Maxsalary = app.jobPost?.Maxsalary ?? 0,
                    EmploymentType = app.jobPost?.EmploymentType ?? "N/A",
                    companyName = app.jobPost?.recruiter?.company?.CompanyName ?? "N/A",
    ...
    public async Task UpdateSeekerDetails(string username, SeekerUpdateDto dto, ClaimsPrincipal userPrincipal)
    {
        try
        {
            ...
            seeker.About = dto.About;
            seeker.Gender = dto.Gender;
            seeker.DateOfBirth = dto.DateOfBirth;
            seeker.ConnectionId = dto.ConnectionId;
    ...
    public async Task UpdateSeekerExperiences(string username, SeekerExperienceUpdateDto dto, ClaimsPrincipal userPrincipal)
    {
        try
        {
            ...
            var claimUsername = userPrincipal?.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            if (claimUsername == null || !claimUsername.Equals(username, StringComparison.OrdinalIgnoreCase))
                throw new UnauthorizedAccessException("You are not authorized to update this seeker.");
            ...
            var existingExperiences = await _experienceRepository.GetAll();
            var seekerExperiences = existingExperiences
                .Where(e => e.SeekerId == seeker.guid)
                .ToList();

            // Delete old experiences
            foreach (var exp in seekerExperiences)
            {
                await _experienceRepository.Delete(exp.Guid);
            }

          
            if (dto.experienceDetails != null)
            {
                var newExperiences = dto.experienceDetails.Select(detail => new Experience
                {
                    Guid = Guid.NewGuid(),
            ...
            _dataLogger.Info($"Updated experiences for seeker {username} (ID: {seeker.guid})");
        }
        catch (RecordNotFoundException ex)
        {
            _errorLogger.Error("Seeker or user not found during experience update", ex);
            throw;
        }
        catch (Exception ex)
        {
            _errorLogger.Error("Failed to update seeker experiences", ex);
            throw new UpdateException("Seeker experiences could not be updated", ex);
        }
    }

    public async Task UpdateSeekerEducations(string username, SeekerEducationUpdateDto dto, ClaimsPrincipal userPrincipal)
{
    try
    {
        // Validate user identity
        ...
        var existingEducations = await _educationRepository.GetAll();
        var seekerEducations = existingEducations
            .Where(e => e.SeekerId == seeker.guid)
            .ToList();

        // Delete old educations
        foreach (var edu in seekerEducations)
        {
            await _educationRepository.Delete(edu.guid);
        }

        // Add new educations
        if (dto.educationDetais != null)
        {
            var newEducations = dto.educationDetais.Select(detail => new Education
            {
                guid = Guid.NewGuid(),
        ...
        _dataLogger.Info($"Updated education records for seeker {username} (ID: {seeker.guid})");
    }
    catch (RecordNotFoundException ex)
    {
        _errorLogger.Error("Seeker or user not found during education update", ex);
        throw;
    }
    catch (Exception ex)
    {
        _errorLogger.Error("Failed to update seeker education records", ex);
        throw new UpdateException("Seeker education could not be updated", ex);
    }
}
    ...
    public async Task<Seeker> GetSeekerByUsername(string username)
    {
        try
        {
