...
public class JobPostService : IJobPostService
{
    ...
    public async Task<JobPostRegisterResponseDto> AddJobPost(JobPostResponse jobPostDto)
    {
        try
        {
            var result = await _transactionalJobPostService.AddJobPostAsync(jobPostDto);
            await _seekerNotificationService.NotifySeekersAsync(result);
    ...
    public async Task<PagedResult<JobPostDto>> GetPagedJobPosts(PageDataDto pageDataDto)
    {
        try
        {
            ...
            var totalCount = await _jobPostRepository.GetAll();
    ...
    public async Task<PagedResult<JobPostDto>> GetJobPostsMatchingProfile(Guid SeekerID, int pageNumber, int pageSize)
    {
        try
        {
            ...
            var scoredPosts = allPosts
                .Where(post => post.requiredSkills != null && post.requiredSkills.Any())
                .Select(post =>
                {
                    var requiredSkillIds = post.requiredSkills.Select(rs => rs.SkillId).ToHashSet();
                    var matchedSkills = requiredSkillIds.Intersect(seekerSkillIds).Count();
                    return new { JobPost = post, MatchScore = matchedSkills };
                })
                .Where(x => x.MatchScore >= 1) // explicitly at least one match
                .OrderByDescending(x => x.MatchScore)
                .OrderByDescending(x => x.JobPost.PostedDate)
                .ToList();
    ...
    public async Task<PagedResult<JobApplication>> GetJobApplicationsByPost(Guid jobPostId, PageDataDto pageDataDto, ClaimsPrincipal user)
    {
        try
        {
            // 1. Get recruiter user
            var username = user?.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            var users = await _userRepository.GetAll();
            var userId = users.FirstOrDefault(u => u.Username == username)?.guid;
            if (userId == null)
            ...
            // 3. Scoring applicants based on skill match (optional)
            var requiredSkillIds = post.requiredSkills?.Select(rs => rs.SkillId).ToHashSet() ?? new();

            var applications = post.jobApplications ?? new List<JobApplication>();
            var scoredApplications = applications.Where(app => !app.IsDeleted && app.seeker != null)
                .Select(app => new
                {
                    Application = app,
                    MatchScore = app.seeker!.seekerSkills?.Select(ss => ss.SkillId).Intersect(requiredSkillIds).Count() ?? 0
                })
                .Where(x => x.MatchScore >= 1)
                .OrderByDescending(x => x.MatchScore)
                .ToList();

            var totalCount = scoredApplications.Count;

            // 4. Paginate and return raw JobApplication objects
            var pagedApplications = scoredApplications
                .Skip((pageDataDto.pageNumber - 1) * pageDataDto.pageSize)
                .Take(pageDataDto.pageSize)
        ...
        {
            _errorLogger.Error($"Error in GetJobApplicationsByPost for postId {jobPostId}", ex);
            throw new FetchDataException($"Failed to fetch job applications: {ex.Message}");
        }
    }
    ...
    public async Task<JobPostDto> UpdateJobPost(Guid postId, JobPostUpdateDto updatedPostDto, ClaimsPrincipal user)
    {
        try
        {
            ...
            existingPost.Minsalary = updatedPostDto.Minsalary;
            existingPost.Maxsalary = updatedPostDto.Maxsalary;
    ...
    public async Task UpdateResponsibilities(Guid postId, ResponsibilitiesUpdateDto updatedResponsibilities, ClaimsPrincipal user)
    {
        try
        {
            ...
            var newEntities = _responsibilityMapper.MapUpdateList(updatedResponsibilities, postId);
    ...
    public async Task UpdateRequirements(Guid postId, RequirementsUpdateDto updatedRequirements, ClaimsPrincipal user)
    {
        try
        {
            var post = await _jobPostRepository.Get(postId) ?? throw new RecordNotFoundException("Job post not found");
            ...
            var newEntities = _requirementMapper.MapUpdateList(updatedRequirements, postId);
    ...
    public async Task UpdatePostSkills(Guid postId, SkillsUpdateDto updatedSkills, ClaimsPrincipal user)
    {
        try
        {
            var post = await _jobPostRepository.Get(postId) ?? throw new RecordNotFoundException("Job post not found");
            ...
            var missingSkills = updatedSkills.UpdatedSkills!
            ...
            var matchedSkills = allSkills
                .Where(s => updatedSkills.UpdatedSkills!.Any(us => us.Name.Equals(s.Name, StringComparison.OrdinalIgnoreCase)))
                .ToList();
    ...
    public async Task<IEnumerable<JobPostDto>> FilterJobPostsByLocation(string location)
    {
        ...
        var filtered = allPosts
            .Where(p => !p.IsDeleted &&
                        p.locations != null &&
                        p.locations.Any(loc =>
                            loc.Name.Contains(location, StringComparison.OrdinalIgnoreCase)))
            .Select(p => jobPostResMapper.MapToDto(p))
            .ToList();
    ...
    public async Task<IEnumerable<JobPostDto>> FilterJobPostsBySalary(int? minSalary = null, int? maxSalary = null)
    {
        var allPosts = await _jobPostRepository.GetAll();

        var filtered = allPosts
            .Where(p => !p.IsDeleted && IsSalaryInRange(p.Minsalary, p.Maxsalary, minSalary, maxSalary))
            .Select(p => jobPostResMapper.MapToDto(p))
            .ToList();
    ...
    private bool IsSalaryInRange(int postMin, int postMax, int? filterMin, int? filterMax)
    {
        return
            (!filterMin.HasValue || postMax >= filterMin.Value) &&
            (!filterMax.HasValue || postMin <= filterMax.Value);
    }
