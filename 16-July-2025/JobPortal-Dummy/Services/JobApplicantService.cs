...
public class JobApplicantService : IJobApplicantService
{
    ...
    public async Task<IEnumerable<JobApplication>> GetApplicationsByPostId(Guid postId)
    {
        try
        {
            ...
            var filteredApplications = allApplications
                .Where(app => app.JobPostId == postId && !app.IsDeleted)
                .ToList();
        ...
        {
            _errorLogger.Error($"Error retrieving applications for JobPostId = {postId}", ex);
            throw new FetchDataException("Error fetching job applications for the given post.");
        }
    }

    public async Task<JobApplication> UpdateStatus(Guid appId, StatusUpdate statusUpdate)
    {
        try
        {
            var application = await _jobApplicationRepository.Get(appId);

            if (application == null || application.IsDeleted)
            {
                throw new RecordNotFoundException("Job application not found or already deleted.");
            }

            application.Status = statusUpdate.status;


            await _jobApplicationRepository.Update(appId, application);

            _dataLogger.Info($"Job application status updated: ApplicationId = {appId}");

            return application;
        }
        catch (RecordNotFoundException ex)
        {
            _errorLogger.Error("Job application not found during status update", ex);
            throw;
        }
        catch (Exception ex)
        {
            _errorLogger.Error($"Failed to update job application status for ApplicationId = {appId}", ex);
            throw new UpdateException("Failed to update application status", ex);
        }
    }

    public async Task<bool> CheckIfApplicationExists(Guid postId, Guid seekerId)
    {
        try
        {
            var allApplications = await _jobApplicationRepository.GetAll();

            var exists = allApplications.Any(app =>
                app.JobPostId == postId &&
        ...
        {
            _errorLogger.Error($"Error checking existing application for PostId = {postId} and SeekerId = {seekerId}", ex);
            throw new FetchDataException("Error checking existing job application.");
        }
    }
