...
public class CompanyService : ICompanyService
{
    ...
    public async Task<IEnumerable<string>> GetAllCompanies()
    {
        try
        {
            ...
            _dataLogger.Info("Fetched all company names.");
            return companyNames;
        }
        catch (Exception ex)
        {
            _errorLogger.Error("Failed to fetch company names", ex);
            throw new FetchDataException("Unable to retrieve company names at the moment.", ex);
        }
    }
    ...
    public async Task<Company> GetCompanyByRecruiterId(Guid recruiterId)
    {
        try
        {
            ...
            var company = await _repository.Get(recruiter.CompanyId);
            if (company == null || company.IsDeleted)
                throw new RecordNotFoundException("Company not found for this recruiter.");

            _dataLogger.Info($"Fetched company for recruiterId={recruiterId}");
            return company;
        }
        catch (RecordNotFoundException ex)
        {
            _errorLogger.Error("Company or recruiter not found.", ex);
            throw;
        }
        catch (Exception ex)
        {
            _errorLogger.Error("Failed to fetch company by recruiterId", ex);
            throw new FetchDataException("Failed to fetch company by recruiter id", ex);
        }
    }
