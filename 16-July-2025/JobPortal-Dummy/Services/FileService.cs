...
public class FileService : IFileService
{
    ...
    public FileService(IRepository<Guid, FileModel> fileRepository, IRepository<Guid, Seeker> seekerRepository, IBlobService blobService)
    {
    ...
    public async Task<FileUploadResponseDto> UploadFileAsync(FileUploadDto fileUploadDto)
    {
        try
        {
            ...
            if (!hasApplied)
                throw new ValidationException("Seeker must apply to the job before uploading a resume.");

            using var memoryStream = new MemoryStream();
            await fileUploadDto.Resume.CopyToAsync(memoryStream);
            memoryStream.Position = 0;

            var blobUrl = await _blobService.UploadFileAsync(memoryStream, fileUploadDto.Resume.FileName, fileUploadDto.Resume.ContentType);
    ...
    public async Task<(Stream, string, string)> DownloadFileStreamAsync(FileGetRequestDto requestDto)
    {
        try
        {
            var file = await _fileRepository.GetAll();
            var fileRecord = file.FirstOrDefault(f =>
                f.SeekerId == requestDto.SeekerId &&
                f.JobPostId == requestDto.JobPostId &&
                !f.IsDeleted);

            if (fileRecord == null || string.IsNullOrWhiteSpace(fileRecord.Url))
                throw new RecordNotFoundException("Resume not found for the specified seeker and job post.");

            var blobClient = new BlobClient(new Uri(fileRecord.Url));
            var blobExists = await blobClient.ExistsAsync();

            if (!blobExists)
                throw new RecordNotFoundException("File not found in blob storage.");
            ...
            var fileName = fileRecord.Name ?? "resume.pdf";

            return (stream, fileName, contentType);
        }
        catch (Exception ex)
        {
            _errorLogger.Error("Failed to download resume from blob", ex);
