...
if (string.IsNullOrEmpty(keyVaulturl))
    throw new Exception("Missing KeyVault:url in appsettings.json");

var keyVaultUri = new Uri(keyVaulturl);
var secretClient = new SecretClient(keyVaultUri, new DefaultAzureCredential());
var secret = await secretClient.GetSecretAsync("DefaultConnection");
var connectionString = secret.Value.Value;
var sec = await secretClient.GetSecretAsync("muthusasurl");
var token = sec.Value.Value;
builder.Configuration["ConnectionStrings:DefaultConnection"] = connectionString;
builder.Configuration["AzureBlob:ConnectionSasUrl"] = token;
...
builder.Services.AddTransient<IRepository<Guid, Experience>, ExperienceRepository>();
builder.Services.AddTransient<IRepository<Guid, Education>, EducationRepository>();
#endregion
